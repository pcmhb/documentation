...
will update, presenting the selected information. To change the :guilabel:`Day`, :guilabel:`Week`,
...
:guilabel:`Today` button, and the dashboard refreshes, presenting information containing the current
day's information.
...
- :guilabel:`Add Custom Group`: this option displays a drop-down menu with a variety of options to
  ...
  and :guilabel:`IP Address`, to name a few.
...
Entries that contain an error appear on the dashboard in red. If in the :guilabel:`Gantt view`, the
...
When all the information on the pop-up is correct, click :guilabel:`Save & Close`. If the entry no
longer has an error, the entry now appears in gray instead of red.
...
button ad a pop-up appears. Select the desired spreadsheet, and click :guilabel:`Confirm`.
