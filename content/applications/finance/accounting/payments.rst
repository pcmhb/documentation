...
When a new payment is registered via the menu :menuselection:`Customers / Vendors --> Payments`
menu, it is not directly linked to an invoice or bill. Instead, the account receivable or the
account payable is matched with the **outstanding account** until it is manually matched with its
...
allows you to process them all one by one, matching all payments and invoices in one place. You can
...
--> Aged Receivable / Aged Payable`. You can see all transactions that have not been reconciled yet
for that partner, and when you select a customer or vendor, the :guilabel:`Reconcile` option is
displayed.
